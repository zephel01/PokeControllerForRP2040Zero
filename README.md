...
2. **ファームウェアを書き込む**:
   - [Releases ページ](https://github.com/zephel01/PokeControllerForRP2040Zero/releases/tag/v1.1.0) から `PokeControllerForRP2040Zero_RP2040Zero_v1.1.0.uf2` をダウンロードします。
...
本ファームウェアは、Switch のコントローラーとして動作しながら、同時に PC との **シリアル通信 (COMポート)** も確立します。
TeraTerm や Arduino IDE のシリアルモニタ等で、RP2040-Zero の COM ポートを開く (ボーレート: 115200) と、以下の情報を確認できます。

- UART 受信データの生ログ (`RX: ...`)
- USB 接続/切断イベント (`USB Mounted/Unmounted`)
- エラー情報 (`Buffer Overflow` 等)
